...
$Id: create_customer_pdf, v1.5 2013/02/12 Paul Cockerill (osc forum id PaulMC)
...
$model_box_length = PRODUCT_MODEL_BOX_WIDTH;
// ------------------------------------------
...
// Security check end //
	...
	$date = (PDF_INV_DATE == 'order') ? tep_date_long($order->info['date_purchased']) : strftime(DATE_FORMAT_LONG);
...
//Page header
 function RoundedRect($x, $y, $w, $h,$r, $style = '')
    ...
    function Header()
    {
    global $HTTP_GET_VARS, $pos_y, $com_name_color, $com_address_color, $com_email_color, $com_web_address_color, $inv_num_id_date_color, $invoice_line,
    ...
    //$this->Ln(0);
    $this->Cell(150);
	$this->MultiCell(50, 4, tep_html_entity_decode(STORE_NAME_ADDRESS),0,'L');
    ...
    if (DISPLAY_PDF_TAX_NUMBER == 'true'){
		...
		//$this->Ln(0);
		$this->Cell(88);
		$this->MultiCell(110, 6, tep_html_entity_decode(PDF_TAX_NAME) . ' ' . PDF_TAX_NUMBER,0,'R');
		$vat_shift_y = 10;
		$pos_y += 10;
    }
	...
	//$this->Ln(0);
    $this->Cell(88);
	$this->MultiCell(110, 6, tep_html_entity_decode(PDF_INV_EMAIL) . STORE_OWNER_EMAIL_ADDRESS,0,'R');
	...
	//$this->Ln(0);
    $this->Cell(88);
	$this->MultiCell(110, 6, tep_html_entity_decode(PDF_INV_WEB) . $web_address,0,'R');
	...
	$this->Text(9, $pos_y+4 ,tep_html_entity_decode(PRINT_INVOICE_TITLE) . (int)$HTTP_GET_VARS['order_id'] );
	$this->Text(9, $pos_y+9 ,$date );
	...
	$this->SetFont(PDF_INV_CORE_FONT,'BI',15);
	$this->SetTextColor($invoice_line[0],$invoice_line[1],$invoice_line[2]);
	$this->Text(23,$pos_y+1.5,tep_html_entity_decode(PRINT_INVOICE_HEADING));
	//$this->SetDrawColor($invoice_line[0],$invoice_line[1],$invoice_line[2]);
	$this->Cell(38);
...
if(findextension(PDF_INVOICE_IMAGE) == 'ai' || findextension(PDF_INVOICE_IMAGE) == 'eps'){
	...
	$pdf->SetFont(PDF_INV_CORE_FONT,'B',10);
	$pdf->Text(13,$pos_y+13, tep_html_entity_decode(PRINT_INVOICE_ORDERNR));	
	$pdf->SetFont(PDF_INV_CORE_FONT,'',10);
	$pdf->Text(39,$pos_y+13, (int)$HTTP_GET_VARS['order_id']);	
	// Draw Date of Order Text
	$pdf->SetFont(PDF_INV_CORE_FONT,'B',10);
	$pdf->Text(63,$pos_y+13, tep_html_entity_decode(PRINT_INVOICE_DATE));	
	$pdf->SetFont(PDF_INV_CORE_FONT,'',10);
	$pdf->Text(88,$pos_y+13, tep_date_short($order->info['date_purchased']));	
	// Draw Payment Method Text
	$pdf->SetFont(PDF_INV_CORE_FONT,'B',10);
	$pdf->Text(122,$pos_y+13, tep_html_entity_decode(ENTRY_PAYMENT_METHOD));
	$pdf->SetFont(PDF_INV_CORE_FONT,'',10);
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
    ...
    // Product line totals
	$pdf->MultiCell(20,7,$prod_line_sym.$price_ex,1,'R');
	$pdf->SetY($Y_Table_Position);
	$pdf->SetX(141);
	$pdf->MultiCell(20,7,$prod_line_sym.$price_inc,1,'R');
	$pdf->SetY($Y_Table_Position);
	$pdf->SetX(161);
	$pdf->MultiCell(20,7,$prod_line_sym.$subt_ex,1,'R');
	$pdf->SetY($Y_Table_Position);
	$pdf->SetX(181);
	$pdf->MultiCell(20,7,$prod_line_sym.$subt_inc,1,'R');
	$Y_Table_Position += 7;
    ...
    if ($Y_Table_Position>265){
        ...
        $pdf->AddPage();
        // Table position, under Fields Name
        $Y_Table_Position = 65+$vat_shift_y;
