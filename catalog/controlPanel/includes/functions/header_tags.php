...
function AddMissingPages($languages_id = 1, $languages)
{
  ...
  $pageTags_query = tep_db_query("select * from " . TABLE_HEADERTAGS . ' where language_id = ' . (int)$languages_id);
...
{   
   ...
   if (NotDuplicatePage($fp, $file))
   {
      ...
      $incArray['line'] .= "    if (! ReadCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['PHP_SELF']), '')) {". "\n";
      $incArray['line'] .= sprintf("      \$header_tags_array = tep_header_tag_page(FILENAME_%s);\n",$fileUpper );
      $incArray['line'] .= "      WriteCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['PHP_SELF']), '');\n    }\n  break;\n";
...
{   
   ...
   { 
     ...
     $incArray['pseudo'] .= "    if (! ReadCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['PHP_SELF']), \$getID)) {" . "\n";
     $incArray['pseudo'] .= "      if (isset(\$parts[0])) {" . "\n";
     $incArray['pseudo'] .= "       \$found = false;" . "\n";
     $incArray['pseudo'] .= "       \$name = FILENAME_" . $fileUpper . " . \"?\" . \$parts[0] . \"=\";" . "\n";
     $incArray['pseudo'] .= "       \$pageTags_query = tep_db_query(\"select * from \" . TABLE_HEADERTAGS . \" where page_name like '\" . tep_db_input(\$name) . \"%' and language_id = '\" . (int)\$languages_id . \"'\");" . "\n";
     ...
     $incArray['pseudo'] .= "               WriteCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['PHP_SELF']), \$getID);" . "\n";
     ...
     $incArray['line'] = sprintf("        \$header_tags_array = tep_header_tag_page(FILENAME_%s);\n",$fileUpper );
     $incArray['line'] .= "        WriteCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['PHP_SELF']), \$getID);" . "\n";
     $incArray['line'] .= "      }  "  . "\n";    
   }
