...
/*****************************************************************
Used to retrive a link for a tag cloud keyword
*****************************************************************/
function GetHTSTagCloudLink($keyword, $languages_id) {
    ...
    $hts_tags_query = tep_db_query("select p.products_id from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where ( pd.products_head_keywords_tag like '%" . $kwrd . "%' or pd.products_name like '%" . $kwrd . "%' ) and p.products_status = 1 " . $quotes . " and pd.language_id = " . (int)$languages_id . " limit 1");
